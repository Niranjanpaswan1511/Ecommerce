...
namespace OjasMart
{
    ...
    public class WebService : System.Web.Services.WebService
    {
        ...
        public string Registration(string Mobile, string Email, string Name, string Password ,string RefferCode)
        {
        ...
        [WebMethod]
        [ScriptMethod(ResponseFormat = ResponseFormat.Json)]
        public string LastWeekAddedProductNew()
        {
            int countcheck = 0;
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        ...
                        obj.Add(new getoffer { OfferId = Convert.ToInt32(row["SrNo"].ToString()), OfferCode = row["OfferCode"].ToString(),
                            Image = imgurl , ItemCode = row["ItemCode"].ToString(), PurchaseAmount = row["PurchaseAmount"].ToString(),
                            CashBackAmount = row["CashBackAmount"].ToString(), ValidStartDate = row["ValidStartDate"].ToString(),
                            ValidEndDate = row["ValidEndDate"].ToString(), points = row["points"].ToString() ,
                            AmountPerPoint = row["AmountPerPoint"].ToString(), CompanyCode = row["CompanyCode"].ToString(),
                            CenterCode = row["CenterCode"].ToString(), IsFirstPurchase = row["IsFirstPurchase"].ToString(),
                            IsfreeItem = row["IsfreeItem"].ToString(), FreeItemCode = row["FreeItemCode"].ToString() ,
                            FreeQuantity = row["FreeQuantity"].ToString(), OfferFor = row["OfferFor"].ToString(),
                            DiscountType = row["DiscountType"].ToString(), DiscountValue = row["DiscountValue"].ToString(),
                            PromoCode = row["PromoCode"].ToString(),
                            ApplyMRPFrom = row["ApplyMRPFrom"].ToString(),
                            ApplyMRPTo = row["ApplyMRPTo"].ToString(),
                        });
